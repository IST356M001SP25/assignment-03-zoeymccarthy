...
if file:
    ...
    for line in text.split("\n"):#split the text by every new line
        st.session_state.total_lines += 1
        line = line.strip()
    ...
    st.session_state.finalCount += 1
    st.session_state.total_lines += count
    #summarize
    for s in st.session_state.summaries:
        st.info(s, icon="💾")
    st.success(f"{st.session_state.fileCount} files processed, {st.session_state.total_lines} total lines procesed")